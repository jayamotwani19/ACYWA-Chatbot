...
class Assistant:
    ...
    def create_chain(self):
        ...
        prompt = ChatPromptTemplate.from_messages([
            ...
            ("system", 
             ...
             "\n10. If a relevant link is available in the data for a specific query, always provide it."),
    ...
    def process_chat(self, question):
        ...
        # Embed links in the response
        main_answer = self.embed_links(main_answer)

        self.log_chat_history(question, main_answer)
    ...
    # Embed links in the response
    def embed_links(self, response):
        # Dictionary of keywords and their corresponding links
        links = {
            "ABS": "https://www.abs.gov.au",
            "other keyword": "https://example.com",
            # Add more keywords and their URLs as needed
        }
        
        # Replace keywords in the response with Markdown links
        for keyword, url in links.items():
            response = response.replace(keyword, f"[{keyword}]({url})")
...
def chat():
    ...
    main_response = assistant.process_chat(user_message)
    return jsonify({
        "reply": main_response
    })
...
# Define endpoint for downloading chat logs as TXT file
@app.route("/download_txt_logs", methods=["GET"])
def download_txt_logs():
    try:
